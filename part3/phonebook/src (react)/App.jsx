...
const App = () => {
	...
	const handleAddPerson = (event) => {
		...
		// Person name already present, ask to update number
		if (sameNamePersons.length > 0) {
			let samePerson = sameNamePersons[0]
			if (!window.confirm(`${samePerson.name} is already added to phonebook, replace the number?`)) return
			personService.update(samePerson.id, { ...samePerson, number: newNumber })
				.then(updatedPerson => {
		...
		// New person, finding first free id
